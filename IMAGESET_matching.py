# Script to retrieve the IMAGESET ID of files by matching the filenames
...
# and then parse the html to get the IMAGESET ID.
...
for file in files:
	...
	if file[-14:-12] == "00":
		...
		IMAGESET_ID = re.search("; IMAGESETID (\d*)",html)

		if IMAGESET_ID:
			
			#output.write(TCP_ID+","+ESTC_ID[7:-1],"\n")
			output2.write(TCP_ID+","+IMAGESET_ID.group(1)+"\n")

		else:
			print("file",TCP_ID,"has no IMAGESET_ID")
